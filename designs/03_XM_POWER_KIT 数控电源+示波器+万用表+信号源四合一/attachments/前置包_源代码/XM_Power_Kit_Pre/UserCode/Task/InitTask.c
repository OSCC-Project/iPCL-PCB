...
uint8_t FLASH_ReadStatusReg1(void);
HAL_StatusTypeDef FLASH_WriteEnable(void);
HAL_StatusTypeDef SPI_SendByte(uint8_t data);
...
void Start_InitTask(void *argument)
{
  ...
  Sys_Init();
  // 初始化失败，进入错误处理循环
  if (!Flash_pass) {              // 如果FLASH有问题，那么直接红屏，没必要进行下一步了。
    GFX_ClearScreen(0xf806);
  }

  if (!Fatfs_pass) {              // 如果FATFS不正常，先格式化全盘
    FLASH_WriteEnable();
    FLASH_CS_LOW();
    SPI_SendByte(FLASH_CMD_CHIP_ERASE);
    FLASH_CS_HIGH();

    while ((FLASH_ReadStatusReg1() & 0x01) != 0) {    // 格式化中，交替刷屏
      GFX_ClearScreen(0xffff);
      HAL_Delay(1000);
      GFX_ClearScreen(0xffe0);
      HAL_Delay(1000);
      GFX_ClearScreen(0xe3e);
      HAL_Delay(1000);
    }

    GFX_ClearScreen(0x7e6);     // 清空成绿色
